...
foreach ($users as $user) {
    ...
    $barber_access = $user['User']['unlimited_barber'];
    $label = ($status == 1) ? 'success' : 'danger';
    $barber_access_label = ($barber_access == 1) ? 'success' : 'danger';
    $label_text = ($status == 1) ? 'Enabled' : 'Disabled';
    $barber_access_text = ($barber_access == 1) ? 'Unlimited' : 'Limited';
    ...
    $records["data"][] = array(
        ...
        '<span class="label label-sm label-' . ($barber_access_label) . '">' . $barber_access_text . '</span>',
