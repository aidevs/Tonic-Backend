...
class PagesController extends AppController {
    ...
    function beforeFilter() {
        parent::beforeFilter();
        $this->Auth->allow(array('tv','tvReservationList','tvWalkinList','ads'));
    }
    ...
    public function tv() {        
        ...
        $shop = $this->User->findByShopSlug($slug);
        if ($shop['User']['unlimited_barber'] != 1){
            $this->redirect('/');
        }
...
//           pr($reservations);die;
        ...
        $AdminShop = $this->Advertisement->find('all', array('conditions' => array('User.role_id' => 1), 'fields' => array('image'),'order'=>'RAND()'));
        if (!empty($AdminShop)) {
        ...
        $currentShopAD = $this->Advertisement->find('all', array('conditions' => array('Advertisement.user_id' => $this->current_shop['User']['id']), 'fields' => array('image'),'order'=>'RAND()'));
        ...
        $this->set(compact('walkins', 'reservations', 'shopAddList', 'adminAddList','shop'));
    }
     ...
     * 	or MissingViewException in debug mode.
     */
    public function tvReservationList() {
        if (!$this->request->is('ajax')) {
            $this->layout = 'tv';
            $this->set('title_for_layout', __($this->current_shop['User']['shop_name'] . ' TV', true));
        } else {
    ...
    public function tvWalkinList() {
        ...
        $this->set(compact('walkins'));
    }
    function ads($shop_id) {
        if ($this->request->is('ajax')) {
            $this->layout = false;
        }
        $adminAddList = array();
        ...
        $currentShopAD = $this->Advertisement->find('all', array('conditions' => array('Advertisement.user_id' => $shop_id), 'fields' => array('image'),'order'=>'RAND()'));
        if (!empty($currentShopAD)) {
            foreach ($currentShopAD as $value) {
                if ($value['Advertisement']['image'] != "") {
                    if (file_exists(WWW_ROOT . 'uploads' . DS . 'advertisement' . DS . $value['Advertisement']['image'])) {
