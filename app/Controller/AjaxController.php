...
class AjaxController extends AppController {
     ...
     *  
     */
    public $uses = array('User', 'Appointment', 'Slot', 'Schedule', 'LunchBreak', 'ReserveSlot', 'Vacation','Week');
...
//            $appointmentsVew = $this->Appointment->find('all', array('conditions' => array('Appointment.status' => 0, 'Appointment.customer_id' => $this->Session->read('Auth.User.id'), 'Appointment.barber_id' => $id, 'Appointment.schedule_id' => $scheduleId, 'Appointment.date' => $seldate), 'order' => array('Slot.time' => 'asc')));
        ...
        $this->set(compact('data','appointments', 'available_slots', 'appointmentsVew'));
...
//            $appointmentsVew = $this->Appointment->find('all', array('conditions' => array('Appointment.status' => 0, 'Appointment.customer_id' => $this->Session->read('Auth.User.id'), 'Appointment.barber_id' => $id, 'Appointment.schedule_id' => $scheduleId, 'Appointment.date' => $seldate), 'order' => array('Slot.time' => 'asc')));
    ...
    public function get_barber_schedule_barber_old($id, $scheduleId, $seldate = null) {
        ...
        $services= $this->BarberService->find('all', array('conditions' => array('Service.status' => 1, 'BarberService.user_id' => $id), 'fields' => array('Service.*')));
    ...
    public function book_schedule() {
        ...
        if (empty($apointment)) {
            $this->Appointment->bindModel(array('hasMany' => array('AppointmentService'=>array('foreignKey' => 'appointment_id', 'className' => 'AppointmentService'),'AppointmentSlot' => array('foreignKey' => 'appointment_id', 'className' => 'AppointmentSlot'))));
            $services= explode(',',$this->request->data['service']);
            $slots= explode(',',$this->request->data['slot_ids']);
            $s=0;
            foreach ($services as $service) {
              $this->request->data['AppointmentService'][$s]['service_id']=$service;  
              $s++;
            }
            $st=0;
            foreach ($slots as $slot) {
              $this->request->data['AppointmentSlot'][$st]['slot_id']=$slot;  
              $st++;
            }
            if ($this->Appointment->saveAll($this->request->data)) {
               
                $appointment['Appointment']['date']=$this->request->data['date'];
                $this->_sendEmailToCustomer($appointment, $slot_detail, $barberDetail,$services);
    ...
    function _sendEmailToCustomer($appointment, $slot, $barberDetail,$services=null) {
        $site_url = SITE_FULL_URL . $this->Session->read('Auth.User.barber') . '/login';
        $srv='';
        if(!empty($services)){
            $this->loadModel('Service');
            $services = $this->Service->find('all', array('conditions' => array('Service.id' => $services)));            
            foreach ($services as $service) {
        ...
        //$site_url = "<a href="" target="_blank" style="text-decoration:none; color:#ffffff; line-height:18px; display:block;" object="link-editable"> Click here to Login </a>";
...
//           $url = Router::url(array('controller' => 'users', 'action' => 'activate', $id, $userDetail['User']['activation_key']), true);
    ...
    function change_schedule() {
        ...
        if (!empty($this->request->data)) {
             $this->Appointment->bindModel(array('belongsTo' => array('Schedule' => array('foreignKey' => 'schedule_id', 'className' => 'Schedule'))));           
             $appointments = $this->Appointment->find('count', array('conditions' => array('Appointment.status' => 0, 'Schedule.user_id' => $this->request->data['Schedule']['user_id'],'Schedule.week_id' => $this->request->data['Schedule']['week_id'], 'Appointment.date >=' => date('y-m-d'))));
             if($appointments > 0){
                 $respons = array('error' => 1, 'msg' =>"Can't be rescheduled as someone has a reserved slot with this barber.");
                 echo json_encode($respons);
                 exit;
             }
            $user= $this->User->find('first',array('conditions'=>array('User.id'=>$this->request->data['Schedule']['user_id']),'fields'=>array('User.service_time')));
            $this->request->data['Schedule']['slot_time']=$user['User']['service_time'];
            $st_time = strtotime($this->request->data['Schedule']['start_time']);
            $end_time = strtotime($this->request->data['Schedule']['end_time']);
            if($this->request->data['Schedule']['slot_time']==0){
                $respons = array('error' => 1, 'msg' => 'Please assign service first.');
    ...
    public function book_barber_schedule() {
        ...
        $this->request->data['email'] = str_replace(' ',  '_', strtolower($this->request->data['name'])).'@'.$shop['User']['shop_slug'].'.dummy'; 
...
//
        ...
        if (empty($apointment)) {
            ...
            $apointmetDataAttay['time'] = $this->request->data['time'];
            $apointmetDataAttay['price'] = $this->request->data['price'];
            ...
            $s=0;
            foreach ($services as $service) {
              $apointmetDataAttay['AppointmentService'][$s]['service_id']=$service;  
              $s++;
            }
            $st=0;
            foreach ($slots as $slot) {
              $apointmetDataAttay['AppointmentSlot'][$st]['slot_id']=$slot;  
              $st++;
            }
            
            if ($this->Appointment->saveAll($apointmetDataAttay)) {
                //$barberDetail = $this->User->findById($apointmetDataAttay['barber_id']);
                $appointment['Appointment']['date']=$this->request->data['date'];
                $customerDetail = $this->User->findById($res['User']['id']);
                //$this->_sendEmailToCustomer_barber($appointment, $slot_detail, $customerDetail,$services);
                try {
...
//           $url = Router::url(array('controller' => 'users', 'action' => 'activate', $id, $userDetail['User']['activation_key']), true);
    ...
    function _sendEmailToCustomer_barber($appointment, $slot, $customerDetail,$services) {
        $site_url = SITE_FULL_URL . $this->Session->read('Auth.User.barber') . '/login';
...
//           $url = Router::url(array('controller' => 'users', 'action' => 'activate', $id, $userDetail['User']['activation_key']), true);
    ...
    function user_barber_change_schedule() {
        ...
        if (!empty($this->request->data)) {
             ...
             $appointments = $this->Appointment->find('count', array('conditions' => array('Appointment.status' => 0, 'Schedule.user_id' => $this->request->data['Schedule']['user_id'],'Schedule.week_id' => $this->request->data['Schedule']['week_id'], 'Appointment.date >=' => date('y-m-d'))));
             if($appointments > 0){
                 $respons = array('error' => 1, 'msg' =>"Can't be rescheduled as someone has a reserved slot with this schedule.");
                 echo json_encode($respons);
                 exit;
             }
            $st_time = strtotime($this->request->data['Schedule']['start_time']);
        ...
        if ($this->request->is('post')) {
            ...
            if (!empty($this->request->data['vacation_from']) && !empty($this->request->data['vacation_to'])) {
                ...
                if ($VacationData['from_date'] >= $VacationData['to_date']) {
                    ...
                    if (empty($query)) {
                        $Vacation = $this->Vacation->findById($vacation_id);
                        $this->Vacation->updateAll($VacationData, array('Vacation.id' => $vacation_id));                        
                        if(strtotime($Vacation['Vacation']['from_date'])!=strtotime($this->request->data['vacation_from']) || strtotime($Vacation['Vacation']['to_date'])!=strtotime($this->request->data['vacation_to'])){
                          $vData['Vacation']['from_date']=$this->request->data['vacation_from'];  
                          $vData['Vacation']['to_date']=$this->request->data['vacation_to'];  
                          $this->_sendNotifyToAdmin($vData); 
                         }
    ...
    public function check_next_available($barberId=null) {
        $now = time();
        $dates=[];
        $data=[];
        for ($i = 1; $i <= 60; $i++) {
            ...
            $is_av = $this->Common->check_barber_next_av($barberId, $currentDayData['Week']['id'], $date);
            if ($is_av['next'] == 1) {
                $data=$is_av;
                break;
}
